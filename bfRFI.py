...
#    """
#    Plots a sigle-sided amplitude spectrum of y(t)
...
#    k = np.arange(n)
#    Time = n/Fs
#    frequency = k/Time                  # two sides frequency range in Hz
#    frequency = (frequency[:n/2])/(1e6) # one sided frequncy range converted to MHz
#    dft = fft(y)/n                      # fft and normalization
#    dft = dft[:n/2]

#    plt.plot(frequency, abs(dft))       # plotting amplitude spectrum
#    plt.xlabel('Freq (MHz)')
#    plt.ylabel('|dft(freq)|')

#    print 'n:', n
#    print 'freq', len(frequency)
#    print 'dft:', len(abs(dft))
...
channel = 2048/2
data = bf0['Data/bf_raw'][channel, : , 0]
...
def plotSpec(y, Fs):
    ...
    for i in range(int(segments)):
        ...
        segmentPol0 = y[offset : offset + segmentSize]
        dft = abs(fft(segmentPol0))/n        # fft and normalization
        #dft = dft[:n/2]
        fullChannel.append(dft)
        print 'segment:', len(segmentPol0)
        
    signal = np.asarray(fullChannel).reshape((int(segments), int(segmentSize)))
    avg = np.average(signal[len(signal)/2:], axis=0)
    k = np.arange(len(avg))
    Time = len(avg)/Fs
#    frequency = k/Time                  # two sides frequency range in Hz
#    frequency = ((Fs) * (frequency[:len(avg)]))/len(avg) # one sided normalized frequncy
#    frequency = frequency*1e6 # one sided frequncy range converted to MHz
    
     
    #frequency =  (np.linspace(-len(avg), len(avg), len(avg)))/len(avg)
    plt.plot(avg)       # plotting amplitude spectrum
    plt.xlabel('Frequency')
    plt.ylabel('')
    #plt.xlabel('Freq (MHz)')
    #plt.ylabel('avg')
    
plotSpec(data, samplingRate)
...
#data1 = bf0['Data/bf_raw'][cleanChannel, :int(10/samplingTime), 0] # samples in 10 seconds

#plt.figure()
#plotSpectrum(bf0['Data/bf_raw'][cleanChannel, :813424, 0], samplingRate)
#plotSpectrum(data1, samplingRate)
